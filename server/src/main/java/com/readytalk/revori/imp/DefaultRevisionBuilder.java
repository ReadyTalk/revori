...
class DefaultRevisionBuilder implements RevisionBuilder {
	private static final Logger log = LoggerFactory
			.getLogger(RevisionBuilder.class);

	private static final ImmutableMap<Class<? extends PatchTemplate>, PatchTemplateAdapter> adapters = ImmutableMap
			.<Class<? extends PatchTemplate>, PatchTemplateAdapter> builder()
			.put(UpdateTemplate.class, new UpdateTemplateAdapter())
			.put(InsertTemplate.class, new InsertTemplateAdapter())
			.put(DeleteTemplate.class, new DeleteTemplateAdapter()).build();
		...
		if (root == null) {
			if (index == 0) {
				...
				if (original == Node.Null) {
					...
					root = Node.delete(token, stack,
							(Node) blaze(index - 1).value, keys[index],
							comparators[index]);
	...
	private Node blaze(int index) {
		...
		if (n == null) {
			if (index == 0) {
				Node root = Node.blaze(blazeResult, token, stack, result.root,
						keys[0], comparators[0]);
				...
				Node root = Node.blaze(blazeResult, token, stack,
						(Node) blaze(index - 1).value, keys[index],
						comparators[index]);
	...
	public void updateIndexTree(Index index, DefaultRevision base,
			NodeStack baseStack, NodeStack forkStack) {
		checkArgument(!index.equals(index.table.primaryKey));

		TableIterator iterator = new TableIterator(reference(index.table),
				base, baseStack, result, forkStack, ConstantAdapter.True,
				new ExpressionContext(null, null), false);
		...
		while (true) {
			...
			case Inserted: {
				...
				for (; i < keyColumns.size() - 1; ++i) {
					...
					setKey(i + Constants.IndexDataBodyDepth,
							Node.find(tree, c, Compare.ColumnComparator).value,
							c.comparator);
				}

				Column c = keyColumns.get(i);
				Node n = blaze(i + Constants.IndexDataBodyDepth,
						Node.find(tree, c, Compare.ColumnComparator).value,
						c.comparator);
			...
			case Deleted: {
				...
				Column c = keyColumns.get(i);
				deleteKey(i + Constants.IndexDataBodyDepth,
						Node.find(tree, c, Compare.ColumnComparator).value,
						c.comparator);
			}
				break;
	...
	private Node makeTree(NodeStack stack, List<Column<?>> columns,
			...
			if (!Compare.Undefined.equals(v)) {
				if (v != null && !c.type.isInstance(v)) {
					throw new ClassCastException(v.getClass().getName()
							+ " cannot be cast to " + c.type.getName());
				}
	...
	public void updateViewTree(View view, DefaultRevision base,
			NodeStack baseStack, NodeStack forkStack) {
		if (log.isDebugEnabled()) { // Because Util.toString() may hurt
									// performance.
			log.debug("update {} diff {}", view, Util.toString(base, result));
		}

		DefaultQueryResult qr = new DefaultQueryResult(base, baseStack, result,
				forkStack, view.query,
		...
		if (view.query.hasAggregates) {
			...
			maxValues = 0;
			for (int i = view.aggregateOffset; i < view.aggregateExpressionOffset; ++i) {
		...
		while (!done) {
			...
			case Inserted: {
				...
				for (; i < keyColumns.size() - 1; ++i) {
					setKey(i + Constants.IndexDataBodyDepth,
							expressions.get(view.primaryKeyOffset + i)
									.evaluate(true),
							keyColumns.get(i).comparator);
				}

				Node n = blaze(i + Constants.IndexDataBodyDepth, expressions
						.get(view.primaryKeyOffset + i).evaluate(true),
						keyColumns.get(i).comparator);

				if (log.isDebugEnabled()) { // Util.toString() potentially
											// expensive.
					log.debug("inserted "
							+ Util.toString(keys, 0,
									Constants.IndexDataBodyDepth + i + 1));
				}
				...
				if (view.query.hasAggregates) {
					...
					for (AggregateAdapter a : aggregates) {
						for (int j = 0; j < a.aggregate.expressions.size(); ++j) {
							values[j] = expressions.get(expressionOffset++)
									.evaluate(true);
						}

						Node old = Node.find((Node) n.value,
								view.columns.get(columnOffset++),
								Compare.ColumnComparator);

						a.add(old == Node.Null ? a.aggregate.function.base()
								: old.value, values);
					}
				} else {
					checkArgument(n.value == Node.Null);
			...
			case Deleted: {
				...
				int index = keyColumns.size() - 1
						+ Constants.IndexDataBodyDepth;
				...
				if (view.query.hasAggregates) {
					...
					for (AggregateAdapter a : aggregates) {
						...
						a.subtract(
								old == Node.Null ? a.aggregate.function.base()
										: old.value, values);
					}
				}

				if (view.query.hasAggregates
						&& (view.query.groupingExpressions.isEmpty() || ((Integer) expressions
								.get(view.aggregateOffset).evaluate(true)) != 0)) {
					blaze(index).value = makeTree(stack, view.columns,
							expressions);
				} else {
					delete(index);
				}
		...
		if (view.query.hasAggregates) {
			...
			while (true) {
				...
				case Deleted: {
					for (int i = 0; i < keyColumns.size(); ++i) {
						setKey(i + Constants.IndexDataBodyDepth, expressions
								.get(view.primaryKeyOffset + i).evaluate(true),
								keyColumns.get(i).comparator);
					}
					...
					int columnOffset = view.aggregateOffset;
					for (AggregateAdapter a : aggregates) {
						a.value = Node.find((Node) find(index).value,
								view.columns.get(columnOffset++),
					...
					if (log.isTraceEnabled()) {
						...
						for (ColumnReferenceAdapter r : qr.expressionContext.columnReferences) {
							filter.append(r.column).append(":").append(r.value)
									.append(" ");
						}

						filter.append(": ").append(qr.test.evaluate(false))
								.append("\n");
					...
					if (qr.test.evaluate(false) == Boolean.FALSE) {
						...
						// should actually just undo whatever operation we did
						// in
						// the first pass, now that we know this query row
						// should
						// not be included. Note that we'll need to operate on a
						// copy of the view rather than the tentative one we
						// just
	...
	private void updateIndexes(View viewToSkip) {
		if (dirtyIndexes && indexBase != result) {
			...
			DiffIterator iterator = new DiffIterator(indexBase.root,
					indexUpdateBaseStack, result.root, indexUpdateForkStack,
					Lists.newArrayList(Interval.Unbounded).iterator(), false,
			...
			while (iterator.next(pair)) {
				if (pair.fork != null) {
					for (NodeIterator indexes = new NodeIterator(
							indexUpdateIterateStack, Node.pathFind(result.root,
									Constants.IndexTable,
									Compare.TableComparator,
									Constants.IndexTable.primaryKey,
									Compare.IndexComparator, pair.fork.key,
									Constants.TableColumn.comparator)); indexes
							.hasNext();) {
						updateIndexTree((Index) indexes.next().key, indexBase,
					...
					for (NodeIterator views = new NodeIterator(
							indexUpdateIterateStack, Node.pathFind(result.root,
									...
									Compare.IndexComparator, pair.fork.key,
									Constants.TableColumn.comparator)); views
							.hasNext();) {
					...
					for (NodeIterator views = new NodeIterator(
							indexUpdateIterateStack, Node.pathFind(result.root,
									...
									Compare.TableComparator,
									Constants.ViewTable.primaryKey,
									Compare.IndexComparator, pair.base.key,
									Constants.TableColumn.comparator)); views
							.hasNext();) {
						View view = (View) views.next().key;
						if (!view.equals(viewToSkip)) {
			...
			for (View v : viewSet) {
				updateViewTree(v, indexBase, indexUpdateBaseStack,
						indexUpdateForkStack);
			}
		}
	...
	private void checkForeignKeys(ForeignKeyResolver resolver) {
		...
		ForeignKeys.checkForeignKeys(new NodeStack(), base, new NodeStack(),
				this, new NodeStack(), resolver, null);
	}
	...
	public void prepareForUpdate(Table table) {
		...
		if (Constants.IndexTable.equals(table)
				...
				|| Node.pathFind(result.root, Constants.IndexTable,
						Compare.TableComparator,
						Constants.IndexTable.primaryKey,
						Compare.IndexComparator, table,
						Constants.TableColumn.comparator) != Node.Null
	...
	private void pathInsert(Table table, Object... path) {
		...
		for (int i = 0; i < columns.size(); ++i) {
			...
			if (i == columns.size() - 1) {
				insertOrUpdate(Constants.IndexDataBodyDepth + i, path[i],
						comparator, tree);
			} else {
	...
	private void addIndex(Index index) {
		if (index.equals(index.table.primaryKey)
				|| Node.pathFind(result.root, Constants.IndexTable,
						Compare.TableComparator,
						Constants.IndexTable.primaryKey,
						Compare.IndexComparator, index.table,
						Constants.TableColumn.comparator, index,
						Constants.IndexColumn.comparator) != Node.Null) {
		...
		updateIndexTree(index, DefaultRevision.Empty, indexUpdateBaseStack,
				indexUpdateForkStack);
	...
	void addView(final View view, DefaultRevision base) {
		...
		view.query.source.visit(new SourceVisitor() {
			public void visit(Source source) {
				if (source instanceof TableReference) {
					...
					if (Node.pathFind(result.root, Constants.ViewTable,
							Compare.TableComparator,
							Constants.ViewTable.primaryKey,
							Compare.IndexComparator, table,
							Constants.TableColumn.comparator, view,
							Constants.ViewColumn.comparator) == Node.Null) {
						isNew[0] = true;
						insert(DuplicateKeyResolution.Throw,
								Constants.ViewTable, table, view,
								Constants.ViewTableColumn, view.table);
					} else if (isNew[0]) {
						throw new RuntimeException(
		...
		if (isNew[0]) {
			...
			updateViewTree(view, base, indexUpdateBaseStack,
					indexUpdateForkStack);
		}
	}
		...
		view.query.source.visit(new SourceVisitor() {
			public void visit(Source source) {
				if (source instanceof TableReference) {
					...
					if (Node.pathFind(result.root, Constants.ViewTable,
							Compare.TableComparator) == Node.Null) {
	...
	private void addForeignKey(ForeignKey constraint) {
		if (Node.pathFind(result.root, Constants.ForeignKeyTable,
				Compare.TableComparator, Constants.ForeignKeyTable.primaryKey,
				Compare.IndexComparator, constraint,
				Constants.ForeignKeyColumn.comparator) != Node.Null) {
			// the specified foreign key is already present -- ignore
			return;
		}

		insert(DuplicateKeyResolution.Throw, Constants.ForeignKeyTable,
				constraint, Constants.ForeignKeyRefererColumn,
				constraint.refererTable);

		insert(DuplicateKeyResolution.Throw, Constants.ForeignKeyTable,
				constraint, Constants.ForeignKeyReferentColumn,
				constraint.referentTable);
	...
	private void doDelete(Object[] keys) {
		...
		deleteKey(
				i - 1 + Constants.IndexDataBodyDepth,
				keys[i],
				keys.length > table.primaryKey.columns.size() + 1 ? Compare.ColumnComparator
						: table.primaryKey.columns.get(i - 1).comparator);
	}

	private void insert(int depth, List<Column<?>> columns, Object[] path) {
	...
	private void insert(DuplicateKeyResolution duplicateKeyResolution,
			Table table, Column<?> column, Object value, Object[] path) {
		prepareForUpdate(table);
	...
	private class MyTableBuilder implements TableBuilder {
		private class MyRowBuilder implements RowBuilder {
			...
			public void init(Object[] keys) {
				...
				insert(DuplicateKeyResolution.Overwrite, path, 0,
						path.length - 2);
			}
		...
		public TableBuilder key(Object... key) {
			...
			insert(DuplicateKeyResolution.Overwrite, row.path, 0,
					key.length + 1);
			return row.up(); // identically, return this (but up() can recycle
								// the RowBuilder)
		}
	...
	public int apply(PatchTemplate template, Object... parameters) {
		try {
			...
			return adapters.get(template.getClass()).apply(this, template,
					parameters.clone());
	...
	public RevisionBuilder delete(Object[] path, int pathOffset, int pathLength) {
	...
	public RevisionBuilder insert(
			DuplicateKeyResolution duplicateKeyResolution, Object[] path,
			int pathOffset, int pathLength) {
		Table table;
		try {
		...
		if (pathLength == columns.size() + 1) {
			...
			throw new IllegalArgumentException(
					"wrong number of parameters for primary key "
							...
							+ java.util.Arrays.toString(myPath));
		}
	...
	private static DefaultRevision getRevision(Object token,
			DefaultRevision basis, Node root) {
